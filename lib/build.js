...
Build.prototype.getMultiBranch = function(opts, callback) {
  var arg0 = typeof arguments[0];
  var arg1 = typeof arguments[1];
  var arg2 = typeof arguments[3];
  var arg3 = typeof arguments[4];
  ...
  if (arg0 === 'string' && (arg1 === 'string' || arg1 === 'number')) {
    if (arg2 === 'object') {
      ...
      opts = {};
      callback = arg2 === 'function' ? arguments[3] : undefined;
    }

    opts.name = arguments[0];
    opts.branch = arguments[1];
    opts.number = arguments[2];
  } else {
    opts = opts || {};
  }
  ...
  try {
    var folder = utils.FolderPath(opts.name);
    // we dont pass it to the utils since brnach name might contains backslashes
    folder.value.push(opts.branch);

    if (folder.isEmpty()) throw new Error('name required');
    if (!opts.number) throw new Error('number required');

    //req.path = '{folder}/job/{branch}/job/{number}/api/json';
    req.path = '{folder}/{number}/api/json';
    req.params = {
      folder: folder.path(),
      branch: opts.branch,
  ...
  return this.jenkins._get(
      req,
      middleware.notFound(opts.name + ' ' + opts.branch + ' ' + opts.number),
      middleware.body,
      callback
  );
};
